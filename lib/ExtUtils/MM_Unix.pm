...
sub tool_xsubpp {
    ...
    my ($xsparentdir, $xsdir);
    my @inc_dirs = @INC;

    # Make sure we pick up the new xsubpp if we're building perl.
    unshift @inc_dirs, $self->{PERL_LIB} if $self->{PERL_CORE};

    foreach my $dir (@inc_dirs) {
        $xsparentdir = $dir;
        $xsdir = $self->catdir($xsparentdir, 'ExtUtils');
